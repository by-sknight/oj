...
// 先检查是否需要执行，如果不用直接返回0
// 然后检查计划是否可行，标记可行的计划
// 检查所有可行的计划执行完是否可以完成目标，如果都不行，就不用额外计算了
// 通过计数的方式，然后叠加所有计划，检查哪些区域只叠加一次，这些区域对应的计划就是必须的，如果没有这个就无法达到目标
// 检查所有的必须的计划是否能达成目标，如果能达成，就不用额外试了
// 找出哪些是可行的计划，但是是非必需的，之后的尝试就在他们中试
// 开始尝试，从只选1个到选择好多个，使用 (1,0,0) 这种方式来标记哪些选择了，使用前/后一个排列函数来快速到下一个排列
//     因为选择数量从小到大，所以找到第一组的时候，就可以直接打印了
